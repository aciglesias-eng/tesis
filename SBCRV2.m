function[n_th_Brayton,n_th_ORC,LCOH, c_el_bus,LCOEn, WNETO, W_Rankine, n_ciclo, n_ex, n_ex_2, n_th, n_en_PEM,n_ex_PEM, T20, m_H2, m_H2O_in, XD, In, Ytotal, Ytotal_PEM]=SBCRV2(nt,nc,Thigh,rc,pmaire)
format long

%ENTRADAS A LA FUNCION-------------------------
% nt            = Eficiencia turbinas
% nc            = Eficiencia comprensores
% Thigh         = Temperatura de alta del ciclo Brayton dado por el recibidor de la torre solar
% rc            = Relacion de presion del Brayton
% pmaire        = porcentaje de flujo masico de aire que entra al cooler

% SALIDAS DE LA FUNCION-------------------------
% W_neto        = Trabajo neto del sistema global (Brayton + ORC)
% W_Rankine     = Trabajo neto del sistema global (ORC)
% n_ciclo       = Eficiencia termica total de los ciclos (Brayton + ORC)
% n_ex          = Eficiencia exergetica global del sistema
%                   (1 - Exergia destruida total / exergia de entrada solar)
% n_th          = Eficiencia termica total de los ciclos (Brayton + ORC + PEM)
% n_en_PEM      = Eficiencia energetica total de la PEM
% n_ex_PEM      = Eficiencia exergetica total de la PEM
% T20           = Temperatura de entrada de la PEM
% m_H2          = Flujo masico de hidrogeno producido
% m_H2O_in      = Flujo masico de agua que requiere la PEM
% XD            = Vector de exergias destruidas y aportadas por componente
% In            = Vector de indicadores de sostenibilidad exergetica:
...
%Entradas SISTEMA BRAYTON----------------------------------------------------------------------------------------------------------------------
    ...
    P0 = 101.325;            % Presion atmosferica (kPa)
    Tlow = 55 + 273.15;      % Temp low (K)--> Guajira
    m_CO2 = 1;               % Flujo masico (kg/s)
    eff = 0.95;              % Eficacia de los recuperadores (aprox para el punto 6)
    Nr = 40;                 % Nr = Numero de segmentos para discretizar los intercambiadores
    r1 = rc;                 % Relacion de presion1 --> r1 = Phigh/Plow (turbinas completas)
    r2 = 1.43;               % Relacion de presion2 --> r2 = Phigh/Pintermedia (Turbina 2)
    Phigh = 25000;           % Presion alta (Salida compresores)
    Plow = Phigh/r1;         % Presion despues de salir de la turbina 2
    Prr = Phigh/r2;          % Presion despues de salir de la turbina 1
    ...
    years = 20;
    gamma = 0.06;
    tipo_bottoming = 'ORC';   % 'ORC' o 'RORC'
    eta_regen_orc  = 0.85;     % solo aplica si tipo_bottoming = 'RORC'

    HX = [460,160,336,125,5; % Libreria de datos de intercambiadores de calor
        799,160,675,150,12;
        837,310,590,200,28;
        1066,310,819,200,40;
        735,310,494,200,20;
...
% Estado de referencia para exergia (estado muerto)
...
% Puntos --------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    % Punto 3
        T3 = T1; P3 = P2;   %Se asume que el recalentador sube la temperatura de punto 3 igual a punto 1
...
% --------------------------------------------------------------------------------------------------------------------------------------------------
%---------------------------------------------------------------------
        % Flujo masico (linea 6AI)
            splitA = (h5-h6)/(h9-h8); %split = m6AI/mtotal= fraccion masica de linea c1

        % Flujo masico (linea 6BI)
            splitB = 1-splitA ;

% --------------------------------------------------------------------------------------------------------------------------------------------------
%---------------------------------------------------------------------
    % Punto 6AO1 / ACOPLE CON ORC-RORC
    P6AO1 = P6AI;
    [T6AO3,A_ORC,W_t3,W_b2,Xd_orc,TPhsmx_orc,Q_cond_vec,Q_evap_vec,Q_regen] = ...
        ORCV2(T6AI,P6AO1,m_CO2*splitA,tipo_bottoming,eta_regen_orc);
    T6AO1 = TPhsmx_orc(14,1);
    h6AO1 = TPhsmx_orc(14,3)*1000;
    s6AO1 = TPhsmx_orc(14,4)*1000;
    x6AO1 = TPhsmx_orc(14,6)*1000;

    % Punto 6AO2 / ACOPLE CON ORC-RORC
    T6AO2 = TPhsmx_orc(15,1);
    P6AO2 = P6AI;
    h6AO2 = TPhsmx_orc(15,3)*1000;
    s6AO2 = TPhsmx_orc(15,4)*1000;
    x6AO2 = TPhsmx_orc(15,6)*1000;

    % Punto 6AO3 / ACOPLE CON ORC-RORC
    P6AO3 = P6AI;
    h6AO3 = TPhsmx_orc(16,3)*1000;
    s6AO3 = TPhsmx_orc(16,4)*1000;
    x6AO3 = TPhsmx_orc(16,6)*1000;
...
% --------------------------------------------------------------------------------------------------------------------------------------------------
% CALCULO ENERGETICO Y EXERGETICO ---------------------------------------------------------------------
...
%---------------------------------------------------------------------
% ANALISIS DE LA TORRE SOLAR (Puntos: 12, 1, 2, 3)
    [nsr, Fi, nfield] = solarReceiver(Thigh, I);  % nsr: eficiencia termica del recibidor solar
                                                  % Fi: fraccion exergetica de la radiacion solar (Petela)
                                                  % nfield: eficiencia del campo heliostato
    % Vector con parametros solares para exportar/reportar
        vector_field = [nsr, Fi, nfield];

    % Exergia solar requerida para entregar q_tt al ciclo
        X_solar = Q_tt*Fi/(nsr*nfield);

    % Exergia perdida en el recibidor por perdidas termicas (Carnot)
        Xloss_sr = Q_tt*(0.95/nsr-1)*(1-T0/Tr);

    % Exergia perdida en el campo solar (perdidas opticas del campo)
        Xloss_field = Q_tt*Fi*(1-nfield)/(nsr*nfield);

    % Exergia destruida en recibidor + heater/reheater (balance exergetico)
        Xd_sr = (X_solar*nfield-Xloss_sr+m_CO2*(x12-x1+x2-x3));

%---------------------------------------------------------------------
    % ANALISIS DE LA TURBINA 1 (Puntos: 1, 2)
        % Potencia de la turbina 1
            W_t1 = m_CO2*(h1-h2);

        % Exergia destruida en la turbina 1
            Xd_t1 = -W_t1+m_CO2*(x1-x2);

%---------------------------------------------------------------------
    % ANALISIS DE LA TURBINA 2 (Puntos: 3, 4)
        % Potencia de la turbina 2
            W_t2 = m_CO2*(h3-h4);

        % Exergia destruida en la turbina 2
            Xd_t2 = -W_t2+m_CO2*(x3-x4);

%---------------------------------------------------------------------
    % ANALISIS DEL COMPRESOR 1 (Puntos: 7, 8)
        % Potencia del compresor 1
            W_c1 = m_CO2*splitA*(h7-h8);

        % Exergia destruida en el compresor 1
            Xd_c1 = -W_c1+m_CO2*splitA*(x7-x8);

%---------------------------------------------------------------------
    % ANALISIS DEL COMPRESOR 2 (Puntos: 6BI, 10)
        % Potencia del compresor 2
            W_c2 = m_CO2*splitB*(h6BI-h10);

        % Exergia destruida en el compresor 2
            Xd_c2 = -W_c2+m_CO2*splitB*(x6BI-x10);

%---------------------------------------------------------------------
    % ANALISIS DEL INTERCAMBIADOR HTR. ---> High recuperator (Puntos: 4, 5, 11, 12)
        % ?rea HTR
            for ii=1:26
                Hr=HX(ii,1); Wr=HX(ii,2); Lpr=HX(ii,3);
                [varout]=HTR(Lpr,Wr,Hr,T12,T11,T4,T5,Phigh,Plow,m_CO2,Q_htr,fluido);
                A_HTR=varout(2);
                if ii==1
                    Amin_HTR=A_HTR;
                else
        ...
        % Exergia destruida en HTR
            Xd_htr = m_CO2*((x4-x5)+(x11-x12));

%---------------------------------------------------------------------
    % ANALISIS DEL INTERCAMBIADOR LTR. ---> Low recuperator (Puntos: 5, 6, 8, 9)
        ...
        % Area LTR
            for ii=1:26
                Hr=HX(ii,1); Wr=HX(ii,2); Lpr=HX(ii,3);
                [varout]=LTR(Lpr,Wr,Hr,T9,T8,T5,T6,Phigh,Plow,Q_ltr,fluido,m_CO2,m_CO2*splitA);
                A_LTR=varout(2);
                if ii==1
                    Amin_LTR=A_LTR;
                else
        ...
        % Exergia destruida en LTR
            Xd_ltr = m_CO2*(x5-x6)+m_CO2*splitA*(x8-x9);

%---------------------------------------------------------------------
    % ANALISIS DEL COOLER (Puntos: 6AO3, 6AO, 13, 14) ACOPLE
        [T6AO, A_cooler, Q_cooler, Xd_cooler, x_loss_cooler, TPhsmx_cooler] = ...
        Cooler_it(T6AO3, Plow, m_CO2*splitA, pmaire);

%--------------------------------------------------------------------------
    % ANALISIS DE LA PEM (Proton Exchange Membrane) (Puntos: 19, 20, 6AO, 7) ACOPLE
        % Potencias -------------------------
            W_Brayton = W_t1+W_t2+W_c1+W_c2;    % W
            W_Rankine = W_t3+W_b2;              % W
            WNETO = W_Brayton+W_Rankine;        % W

        [TPhsmx_PEM, Xd_PEM_TOT, A_HE, Q_HE, ~, ~, m_H2O_in, m_H2] = ...
            PEM_OFICIAL(T6AO, Plow, m_CO2*splitA, T7, W_Rankine);
        T19 = TPhsmx_PEM(3,1);
        T20 = TPhsmx_PEM(4,1);
...
% RESULTADOS
    % Exergia destruida
        ...
        XD(1,13) = Xd_orc(5)/1000;      % Destruida evaporador
        XD(1,14) = Xd_orc(4)/1000;      % Destruida condensador
        XD(1,15) = Xd_orc(1)/1000;      % Destruida pump 2
        XD(1,16) = Xd_orc(6)/1000;      % Loss en el agua
        XD(1,17) = Xd_PEM_TOT(1)/1000;  % Destruida HE
        XD(1,18) = X_solar/1000;        % Exergia aportada por el sistema solar
        XD(1,19) = Xd_PEM_TOT(4);       % Exergia destruida por la PEM
        XD(1,20) = Xd_orc(3)/1000;      % Regenerador
        Xd_total = sum(XD(1,1:19));
        XD(1,21) = Xd_total-X_solar/1000;% Exergia destruida total

        % Eficiencia exergetica ------------------------
            n_ex_2 = (1-(XD(1,21)/(X_solar/1000)))*100;

    % ?reas de los intercambiadores
        Areas(1,1) = Amin_HTR;       % Area HTR
        Areas(1,2) = Amin_LTR;       % Area LTR
        Areas(1,3) = A_ORC(1);       % Area Evaporador
        Areas(1,4) = A_ORC(3);       % Area Condensador
        Areas(1,5) = A_cooler;       % Area Cooler
        Areas(1,6) = A_HE;           % Area Heat Exchanger
        Areas(1,7) = A_ORC(2);       % Area Regenerador
...
% --------------------------------------------------------------------------------------------------------------------------------------------------
% CALCULO AMBIENTAL ---------------------------------------------------------------------
    % Indicadores de sostenibilidad exergetica
        EWR = XD(1,21)*1000/X_solar;    % Exergy waste ratio
        EEF = EWR/(n_ex/100);           % Environmental effect factor
        ESI = 1/EEF;                    % Exergetic sustainability index
...
% CALCULO EXERGOECONOMICO ---------------------------------------------------------------------
    % Potencias en kW
        ...
        Wt3   = abs(W_t3)/1000;
        Wb2   = abs(W_b2)/1000;
        W_PEM = abs(W_Rankine)/1000;

    % Impacto ambiental por componente
        [YCOPROD, Ytotal, Yco_total, Yde_total, Yom_total, Y_componentes,...
            Ytotal_PEM,Yco_total_PEM, Yde_total_PEM, Yom_total_PEM] = ...
            EnvironmentalV22(W1,W2,Wt3,Wc1,Wc2,Wb2,Areas(1,1), Areas(1,2),...
            Areas(1,5), Areas(1,3) ,Areas(1,4), Areas(1,6), Areas(1,7));

    % Factores de actualizacion de costo (CEPCI 2022)
        CEPCI_2022 = 816;
    ...
    % Costo de inversion por componente
        ...
        Z_HTR       = F_HXC * 2681*(Amin_HTR^0.59);
        Z_LTR       = F_HXC * 2681*(Amin_LTR^0.59);
        Z_ORC_EVAP  = F_HXC * 2681*(A_ORC(1)^0.59);
        Z_ORC_REGEN = F_HXC * 2681*(A_ORC(2)^0.59);
        Z_t3        = F_T3  * 4405*(Wt3^0.7);
        Z_b2        = F_PUMP * 1120*(Wb2^0.8);
        Z_ORC_COND  = F_HXS * 2143*(A_ORC(3)^0.514);
        Z_COOLER    = F_HXS * 2143*(A_cooler^0.514);
        Z_HE        = F_HXS * 2143*(A_HE^0.514);
        Z_PEM       = F_PEM * 1000*W_PEM;

    % Tasa de costo de inversion [USD/h]
        ...
        Zdot_ORC_REGEN = Z_ORC_REGEN*f_Z;
        Zdot_t3        = Z_t3*f_Z;
        Zdot_b2        = Z_b2*f_Z;
    ...
    % Costos unitarios externos (frontera del sistema)
        % Costo por unidad de EXERGIA, no de energia [USD/kWh_ex].
        % Ajustar con referencia (LCOE torre solar / costo de agua tratada).
        c_solar = 0.02;   % Exergia solar que entra al heater/reheater
        c_water = 0.002;  % Agua de reposicion de la PEM
    ...
    % Exergias de corrientes [kW]
        ...
        E6AO  =  m_CO2*splitA*TPhsmx_cooler(4,6);
        E19   =  m_H2O_in*TPhsmx_PEM(3,6);
        E20   =  m_H2O_in*TPhsmx_PEM(4,6);

        EH2   = Xd_PEM_TOT(2);
        EO2   = Xd_PEM_TOT(5);
        E_orc = TPhsmx_orc(:,5).*TPhsmx_orc(:,6);   % [kW]
        E17  = E_orc(1);
        E18  = E_orc(2);
        E18r = E_orc(3);
        E15  = E_orc(6);
    ...
    % Incognitas: costo unitario de cada corriente
        names = {'c1','c2','c3','c4','c5','c6','c7','c8','c9','c10','c11','c12', ...
                 'c6BI','c6AI','c6AO3','c6AO','c19','c20', ...
                 'c15','c16','c16r','c17','c18','c18r','cH2','cO2','cel'};
    ...
    % Balances de costo y ecuaciones auxiliares
        % (1) Heater solar: C1 - C12 = costo de la exergia solar gastada aqui
            A(1,idx('c1'))  =  E1;
            A(1,idx('c12')) = -E12;
            b(1) = c_solar*(X_solar/1000)*(Q_heater/Q_tt);

        % (2) Reheater solar: C3 - C2 = costo de la exergia solar gastada aqui
            A(2,idx('c3')) =  E3;
            A(2,idx('c2')) = -E2;
            b(2) = c_solar*(X_solar/1000)*(Q_reheater/Q_tt);

        % (3) Bus electrico comun de turbinas
            A(3,idx('c2'))  =  E2;
            A(3,idx('c1'))  = -E1;
            A(3,idx('c4'))  =  E4;
            A(3,idx('c3'))  = -E3;
        ...
        % (4)-(6) Igualdad de costos unitarios a traves de las turbinas
        ...
        % (18) Mezclador Brayton
            A(18,idx('c11')) =  E11;
            A(18,idx('c9'))  = -E9;
            A(18,idx('c10')) = -E10;
            b(18) = 0;

        % (19)-(24) Ecuaciones auxiliares
            A(19,idx('c4'))   =  1; A(19,idx('c5'))   = -1; b(19) = 0;
            A(20,idx('c5'))   =  1; A(20,idx('c6'))   = -1; b(20) = 0;
            A(21,idx('c6'))   =  1; A(21,idx('c6AI')) = -1; b(21) = 0;
            A(22,idx('c6'))   =  1; A(22,idx('c6BI')) = -1; b(22) = 0;
            A(23,idx('c6AI')) =  1; A(23,idx('c6AO3'))= -1; b(23) = 0;
            A(24,idx('c6AO')) =  1; A(24,idx('c7'))   = -1; b(24) = 0;

        % (25) Agua de reposicion
            A(25,idx('c19')) = 1;
            b(25) = c_water;

        % (26) Oxigeno sin valorizacion economica
            A(26,idx('cO2')) = 1;
            b(26) = 0;

        % (27) Regla auxiliar del regenerador
            A(27,idx('c16r')) =  1;
            A(27,idx('c16'))  = -1;
            b(27) = 0;

    % Diagnostico del sistema de costos
        if rank(A) < n
            warning('Matriz de costos deficiente: rank = %d de %d', rank(A), n);
        end
        fprintf('cond(A) = %.3e\n', cond(A));
    ...
    % Flujos de costo [USD/h]
        Vector_E = [E1; E2; E3; E4; E5; E6; E7; E8; E9; E10; E11; E12; ...
                    E6BI; E6AI; E6AO3; E6AO; E19; E20; ...
                    E15; E16; E16r; E17; E18; E18r; EH2; EO2; W_PEM];

        Cdot = x_costos .* Vector_E;

    % Parametros exergoeconomicos por componente
        Comp_Names = {'Turbina 1'; 'Turbina 2'; 'Compresor 1'; 'Compresor 2'; ...
                      'HTR'; 'LTR'; 'Evaporador RORC'; 'Regenerador RORC'; ...
                      'Turbina ORC'; 'Bomba ORC'; 'Condensador RORC'; ...
                      'Cooler'; 'HE-PEM'; 'PEM'};

        Zdot = [Zdot_t1; Zdot_t2; Zdot_c1; Zdot_c2; ...
                Zdot_HTR; Zdot_LTR; Zdot_ORC_EVAP; Zdot_ORC_REGEN; ...
                Zdot_t3; Zdot_b2; Zdot_ORC_COND; Zdot_COOLER; Zdot_HE; Zdot_PEM];

        Xd_b2_kW    = Xd_orc(1)/1000;
        Xd_t3_kW    = Xd_orc(2)/1000;
        Xd_regen_kW = Xd_orc(3)/1000;
        Xd_cond_kW  = Xd_orc(4)/1000;
        Xd_evap_kW  = Xd_orc(5)/1000;
        ...
        cF_k = [sol.c1; sol.c3; sol.cel; sol.cel; ...
                sol.c4; sol.c5; sol.c6AI; sol.c16; ...
                sol.c15; sol.cel; sol.c16r; sol.c6AO3; ...
                sol.c6AO; sol.cel];

        cP_k = [sol.cel; sol.cel; sol.c8; sol.c10; ...
                sol.c12; sol.c9; sol.c15; sol.c18r; ...
                sol.cel; sol.c18; sol.c17; sol.c6AO; ...
                sol.c20; sol.cH2];
        ...
        den_fk = Zdot + CD_k;
        mask_fk = den_fk > eps;
        f_k(mask_fk) = 100*(Zdot(mask_fk)./den_fk(mask_fk));

    % Indicadores globales
        Cdot_fuel = c_solar * (X_solar/1000);
        E_H2_LHV  = m_H2*LHV_H2/1000;
        W_grid    = max(W_Brayton,0)/1000;
        Zdot_total = sum(Zdot);

        LCOEn    = (Cdot_fuel + Zdot_total) / max(W_grid + E_H2_LHV, eps);
        LCOH     = sol.cH2;
        c_el_bus = sol.cel;

    % Sensibilidad al costo del combustible solar
        % Solo cambia b (la matriz A no depende de c_solar). Para cada valor
        % se reconstruyen los terminos solares, se resuelve y se extrae LCOH,
        % c_el y LCOEn. Sirve para reportar un rango en vez de un solo numero.
        c_solar_vec = (0.01:0.005:0.04)';   % USD/kWh_ex
        Sensibilidad = zeros(numel(c_solar_vec),4);
        for s = 1:numel(c_solar_vec)
            cs   = c_solar_vec(s);
            b_s  = b;
            b_s(1) = cs*(X_solar/1000)*(Q_heater/Q_tt);
            b_s(2) = cs*(X_solar/1000)*(Q_reheater/Q_tt);
            x_s  = A\b_s;
            LCOEn_s = (cs*(X_solar/1000) + Zdot_total)/max(W_grid + E_H2_LHV, eps);
            Sensibilidad(s,:) = [cs, LCOEn_s, x_s(idx('cH2')), x_s(idx('cel'))];
        end
...
% SALIDA DE RESULTADOS ---------------------------------------------------------------------
    ...
    % Hoja Exergoeconomico: componentes
        Datos_Comp = [Comp_Names, num2cell(Zdot), num2cell(ED_k), num2cell(CD_k), ...
                      num2cell(cF_k), num2cell(cP_k), num2cell(f_k)];
        xlswrite('Salidas.xlsx', Datos_Comp, 'Exergoeconomico', 'E2');

    % Hoja Exergoeconomico: indicadores globales
    ...
    % Hoja Exergoeconomico: sensibilidad a c_solar
        Titulos_Sens = {'c_solar [USD/kWh_ex]', 'LCOEn [USD/kWh]', ...
                        'c_H2 [USD/kWh_ex]', 'c_el [USD/kWh_ex]'};
        xlswrite('Salidas.xlsx', Titulos_Sens,  'Exergoeconomico', 'K5');
        xlswrite('Salidas.xlsx', Sensibilidad,  'Exergoeconomico', 'K6');
end
