function [YCOPROD, GWP_elec, Yco_total, Yde_total, Yom_total, Y_componentes, Ytotal_PEM,Yco_total_PEM, Yde_total_PEM, Yom_total_PEM] = EnvironmentalV22( ...
    W_t1, W_t2, W_t3, W_c1, W_c2, W_b2, A_HTR, A_LTR, A_cooler, A_evap, A_cond, A_HE, A_regen)
...
% Decomisionamiento
    ...
    Yde.regen  = peed*mu_eq_mass*M_regen;
    ...
    Yde_block  = Yde.Evap+Yde.HE+Yde.Cond+Yde.HTR+Yde.LTR+Yde.Cooler+Yde.T1...
                +Yde.T2+Yde.T3+Yde.C1+Yde.C2+Yde.B2+Yde.wf+Yde.co2+Yde.CSP+Yde.regen; %
%--------------------------------------------------------------------------
% Comisionamiento
    ...
    Yco.regen  = peed*mu_eq_mass*M_regen;
    ...
    Yco_block  = Yco.Evap+Yco.HE+Yco.Cond+Yco.HTR+Yco.LTR+Yco.Cooler+Yco.T1...
                +Yco.T2+Yco.T3+Yco.C1+Yco.C2+Yco.B2+Yco.wf+Yco.co2+Yco.CSP+Yco.regen; %
%--------------------------------------------------------------------------
...
% Impacto total por componente
    ...
    Y.regen  = Yco.regen + Yde.regen + Yom.regen; 
...
Y_componentes = [Y.Evap,Y.Cond,Y.HTR,Y.LTR,Y.Cooler,Y.HE,Y.T1,Y.T2,Y.T3, ...
                 Y.C1,Y.C2,Y.B2,Y.wf,Y.co2,Y.CSP,Y_PEME,Y.regen];
