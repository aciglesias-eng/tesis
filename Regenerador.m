function [varout]=Regenerador(Lpr,Wr,Hr,T2r,T2,T4,T4r,Phigh,Plow,fm3,Qreg,fluid)        
    ...
    while (swich==1)           
        ...
        %Number of plates
            ...
            Dh=2*ch_gap; %m        
        %Channel velocity
            Vch_hf=fm3/(rho_hf*ch_gap*W/1000*Nch_pass); %m/s
            Vch_cf=fm3/(rho_cf*ch_gap*W/1000*Nch_pass); %m/s
        %Reynolds
        ...
        %Port pressure drop
            Vpt_hf=(fm3/rho_hf)/(pi/4*Dport^2); %m/2
            Vpt_cf=(fm3/rho_cf)/(pi/4*Dport^2); %m/2        
            Dpt_hf=1.3*rho_hf/2*Vpt_hf^2; %N/m2
            Dpt_cf=1.3*rho_cf/2*Vpt_cf^2; %N/m2
        %Exergy destroyed because of pressure drop
            ExDP_hf=fm3*(Dpp_hf+Dpt_hf)/rho_hf; %W
            ExDP_cf=fm3*(Dpp_cf+Dpt_cf)/rho_cf; %W
        %Output
